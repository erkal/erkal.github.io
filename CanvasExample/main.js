...
var $author$project$CanvasExample$Main$init = function (computer) {
	return {
		pointerInGameCoordinates: A2($author$project$Geometry2d$Point2d, 0, 0)
	};
};
...
var $author$project$Camera$Camera = function (a) {
	return {$: 'Camera', a: a};
};
var $elm$core$Basics$atan = _Basics_atan;
var $author$project$Camera$init = function (_v0) {
	var aspectRatio = _v0.aspectRatio;
	var fOVWidth = _v0.fOVWidth;
	var yIsUp = _v0.yIsUp;
	return $author$project$Camera$Camera(
		{
			horizontalAngleOfView: 2 * $elm$core$Basics$atan(0.5),
...
var $author$project$CanvasExample$Main$camera = function (computer) {
	return $author$project$Camera$init(
		{aspectRatio: computer.screen.width / computer.screen.height, fOVWidth: 20, yIsUp: false});
};
var $author$project$Camera$reverseIfYIsUp = F2(
	function (_v0, v) {
		var camera = _v0.a;
		return camera.yIsUp ? (-v) : v;
	});
var $author$project$Camera$screenCoordinatesToCanvasCoordinates = F2(
	function (_v0, _v1) {
		...
		var x = _v1.x;
		var y = _v1.y;
		var c = camera.viewPoint;
		return {
			x: ((x - 0.5) * c.z) + c.x,
			y: (A2(
				$author$project$Camera$reverseIfYIsUp,
				$author$project$Camera$Camera(camera),
				y - 0.5) * c.z) + c.y
		};
	});
	...
	function (computer, message, model) {
		if (message.$ === 'Tick') {
			return _Utils_update(
				model,
				{
					pointerInGameCoordinates: A2(
						$author$project$Camera$screenCoordinatesToCanvasCoordinates,
						$author$project$CanvasExample$Main$camera(computer),
						A2($author$project$Geometry2d$Point2d, 0.5 + (computer.pointer.x / computer.screen.width), 0.5 - (computer.pointer.y / computer.screen.width)))
				});
		} else {
...
var $author$project$SceneCanvas2D$Geometry$move = F2(
	function (_v0, geometry) {
		...
		return _Utils_update(
			geometry,
			{x: geometry.x + dx, y: geometry.y + dy, z: geometry.z + dz});
	});
...
var $author$project$Camera$getHorizontalAngleOfView = function (_v0) {
	var camera = _v0.a;
	return camera.horizontalAngleOfView;
};
...
var $author$project$Camera$getFOVBoundingBoxAtZ = F2(
	function (z, camera) {
		...
		return {
			bottom: $author$project$Camera$getY(camera) + A2($author$project$Camera$reverseIfYIsUp, camera, halfHeightAtZ),
			left: $author$project$Camera$getX(camera) - halfWidthAtZ,
			right: $author$project$Camera$getX(camera) + halfWidthAtZ,
			top: $author$project$Camera$getY(camera) - A2($author$project$Camera$reverseIfYIsUp, camera, halfHeightAtZ)
		};
	});
...
var $author$project$SceneCanvas2D$Geometry$rotatePoint = F2(
	function (angle, p) {
		return _Utils_update(
			p,
			{
				x: (p.x * $elm$core$Basics$cos(angle)) - (p.y * $elm$core$Basics$sin(angle)),
	...
	function (xScale, yScale, geometry) {
		return A2(
			...
			A2(
				...
				_Utils_update(
					geometry,
					{x: xScale * geometry.x, y: yScale * geometry.y})));
	});
...
var $author$project$SceneCanvas2D$view = F4(
	function (fOVWidthInPixels, gameHeightInPixels, camera, _v0) {
		...
		var rotationAndScale = function (_v10) {
			var rotation = _v10.rotation;
			var xScale = _v10.xScale;
			var yScale = _v10.yScale;
			return _List_fromArray(
				[
		...
		var cameraPerspective = function (_v9) {
			...
			var cameraWidthAtZ = cameraBoundingBoxAtZ.right - cameraBoundingBoxAtZ.left;
			var scaleFactor = fOVWidthInPixels / cameraWidthAtZ;
			var _v8 = _Utils_Tuple2(scaleFactor * (x - cameraBoundingBoxAtZ.left), scaleFactor * (y - cameraBoundingBoxAtZ.top));
		...
		var helper = function (_v1) {
			...
			switch (bareDrawable_.$) {
							...
							function (drawable) {
								return helper(
									A2(
										...
										_Utils_Tuple3(geometry_.x, geometry_.y, geometry_.z),
										A2(
											$author$project$SceneCanvas2D$rotateAroundOrigin,
											geometry_.rotation,
											A3($author$project$SceneCanvas2D$scaleAroundOrigin, geometry_.xScale, geometry_.yScale, drawable))));
							},
							elements));
				case 'Rectangle':
						...
						[
							...
							_List_fromArray(
								[
									...
									{family: 'system-ui', size: parameters.size})
								]),
							_Utils_Tuple2(0, 0),
							parameters.text)
						]);
						...
						[
							...
							A2(
								...
								_Utils_ap(
									...
									_Utils_ap(
										...
										_List_fromArray(
											[
												$joakin$elm_canvas$Canvas$Settings$Line$lineWidth(parameters.lineWidth),
							...
							_List_fromArray(
								[
									...
									_Utils_Tuple2(0, 0),
									parameters.radius,
									{clockwise: parameters.clockwise, endAngle: parameters.endAngle, startAngle: parameters.startAngle})
								]))
						]);
				case 'Path':
					var parameters = bareDrawable_.a;
							...
							case 'MoveTo':
								...
								return $joakin$elm_canvas$Canvas$moveTo(
									_Utils_Tuple2(
										...
										A2($author$project$Camera$reverseIfYIsUp, camera, y)));
							case 'ArcTo':
								var start = extension.a.start;
								var end = extension.a.end;
								var radius = extension.a.radius;
								return A3(
									$joakin$elm_canvas$Canvas$arcTo,
									_Utils_Tuple2(
										start.x,
										A2($author$project$Camera$reverseIfYIsUp, camera, start.y)),
									_Utils_Tuple2(
										end.x,
										A2($author$project$Camera$reverseIfYIsUp, camera, end.y)),
									radius);
							default:
								var control1 = extension.a.control1;
								var control2 = extension.a.control2;
								var end = extension.a.end;
								return A3(
									$joakin$elm_canvas$Canvas$bezierCurveTo,
									_Utils_Tuple2(
										control1.x,
										A2($author$project$Camera$reverseIfYIsUp, camera, control1.y)),
									_Utils_Tuple2(
										control2.x,
										A2($author$project$Camera$reverseIfYIsUp, camera, control2.y)),
									_Utils_Tuple2(
										end.x,
										A2($author$project$Camera$reverseIfYIsUp, camera, end.y)));
						}
				...
				default:
					...
					var textureWidth = $joakin$elm_canvas$Canvas$Texture$dimensions(texture).width;
					var textureHeight = $joakin$elm_canvas$Canvas$Texture$dimensions(texture).height;
...
var $author$project$SceneCanvas2D$toHtml = F2(
	function (_v0, drawables) {
		var sceneWidthInPixels = _v0.sceneWidthInPixels;
	...
	function (computer, model) {
		return $rtfeldman$elm_css$Html$Styled$fromUnstyled(
			A2(
				...
				{
					camera: $author$project$CanvasExample$Main$camera(computer),
					sceneWidthInPixels: $elm$core$Basics$round(computer.screen.width)
				},
				_List_fromArray(
					[
						...
						A2(
						$author$project$SceneCanvas2D$moveX,
						$elm$core$Basics$sin(3 * computer.clock),
						...
						A2(
						$author$project$SceneCanvas2D$moveY,
						model.pointerInGameCoordinates.y,
						A2(
							$author$project$SceneCanvas2D$moveX,
							model.pointerInGameCoordinates.x,
...
var $author$project$CanvasExample$Main$main = $author$project$Play$simpleApplication(
	{hasTape: true, init: $author$project$CanvasExample$Main$init, initialConfigurations: $author$project$CanvasExample$Main$initialConfigurations, update: $author$project$CanvasExample$Main$update, view: $author$project$CanvasExample$Main$view});
