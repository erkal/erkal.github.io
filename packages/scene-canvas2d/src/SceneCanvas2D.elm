...
path : { stroke : Maybe Color, lineWidth : Float, fillColor : Maybe Color } -> List PathExtension -> Drawable
path parameters pathExtensions =
    Drawable defaultGeometry (Path parameters pathExtensions)


line : { stroke : Maybe Color, lineWidth : Float, fillColor : Maybe Color } -> Point -> Point -> Drawable
line props p q =
    path
        props
...
polyline : { stroke : Maybe Color, lineWidth : Float, fillColor : Maybe Color } -> List Point -> Drawable
...
toHtml : { sceneWidthInPixels : Int, camera : Camera } -> List Drawable -> Html msg
toHtml { sceneWidthInPixels, camera } drawables =
    let
        ...
        renderables =
            List.concatMap (view width height camera) drawables
    in
...
view fOVWidthInPixels gameHeightInPixels camera (Drawable geometry bareDrawable) =
    let
        ...
        cameraPerspective : Geometry -> List Canvas.Settings.Advanced.Transform
        ...
        geometryTransform : Geometry -> List Canvas.Settings.Advanced.Transform
        geometryTransform geometry__ =
            cameraPerspective geometry__ ++ rotationAndScale geometry__

        helper : Drawable -> List Renderable
