...
module Builder.Build exposing
  ...
  --
  , findModulePath
  , lensCompilationDataForElmViz
  --
  , CompilationDataForElmViz
  )
...
import Compiler.Compile as Compile exposing (ModuleCompilationDataForElmViz)
...
type LocalState e f g h = LocalState
  ...
  {- compilationDataForElmViz -} (CompilationDataForElmViz)


type alias CompilationDataForElmViz =
  List ModuleCompilationDataForElmViz
...
lensCompilationDataForElmViz : Lens (State e f g h) (CompilationDataForElmViz)
lensCompilationDataForElmViz =
...
checkModule ((Env root projectType _ _ _ _) as env) foreigns resultsMVar name status () =
  case status of
      ...
      case depsStatus of
        DepsChange ifaces ->
          IO.bind (File.readUtf8 path) <| \source ->
            case Parse.fromByteString projectType source of
              Right modul -> compile_ForElmViz env local source ifaces modul
              Left err ->
                IO.return <| RProblem <|
    ...
    SChanged ((Details.Local path time deps _ _ lastCompile) as local) source ((Src.Module _ _ imports _ _ _ _ _) as modul) ->
      ...
      case depsStatus of
        DepsChange ifaces ->
          compile_ForElmViz env local source ifaces modul

        DepsSame same cached ->
          IO.bind (loadInterfaces root same cached) <| \maybeLoaded ->
          case maybeLoaded of
            Nothing     -> IO.return RBlocked
            Just ifaces -> compile_ForElmViz env local source ifaces modul
...
compile_ForElmViz : Env -> Details.Local -> String -> Map.Map ModuleName.Raw I.Interface -> Src.Module -> IO e f g h Result
compile_ForElmViz (Env root projectType _ buildID _ _) (Details.Local path time deps main lastChange _) source ifaces modul =
  let
    pkg = projectTypeToPkg projectType
    d = Compile.compile_ForElmViz pkg ifaces modul
    rProb err =
        IO.bind (IO.modifyLens lensCompilationDataForElmViz (\old -> d :: old)) <| \_ ->
        IO.return <| RProblem (Error.Module (Src.getName modul) path time source err)
  in
  case d of
    Compile.Success canonical annotations objects ->
      let name = Src.getName modul in
      let iface = I.fromModule pkg canonical annotations in
      let elmi = Stuff.elmi root name in
      IO.bind (File.writeBinary Opt.bLocalGraph (Stuff.elmo root name) objects) <| \_ ->
    ...
    Compile.FailedAtCanonicalize err -> rProb err
    Compile.FailedAtTypeCheck err _  -> rProb err
    Compile.FailedAtNitpick err _ _  -> rProb err
    Compile.FailedAtOptimize err _ _ -> rProb err
